...
from .validate import normalize_header, validate_headers


def _sheet_rows(path: Path) -> Tuple[List[str], List[List[Any]]]:
...
def load_boxes_packed(path: Path, file_label: str = "Boxes_Packed_by_Worker.xlsx"):
    headers, raw_rows = _sheet_rows(path)
    validate_headers(file_label, headers, BOXES_REQUIRED_COLUMNS)
    kept = []
...
def load_intra_hour(path: Path, file_label: str = "Intra_Hour_Floor_Performance.xlsx"):
    ...
    """
    headers, raw_rows = _sheet_rows(path)
    validate_headers(file_label, headers, INTRA_REQUIRED_COLUMNS)
    kept = []
...
def load_facility_summary(path: Path, file_label: str = "Overall_Summary_by_Packer_and_Date.xlsx"):
    ...
    """
    headers, raw_rows = _sheet_rows(path)
    validate_headers(file_label, headers, SUMMARY_REQUIRED_COLUMNS)
    kept = []
    for row in raw_rows:
        d = _row_dict(headers, row)
        if _is_blank(d.get("Pnp Worker Name")):
